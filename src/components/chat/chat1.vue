...
<div class="chatContent">
    <el-drawer :title='`患者：${name} ${sexStr} ${age}`' size="100%" :visible="drawerVisible" :with-header="false" custom-class="my-drawer" @close="close">
        <div class="chatLeft">
            <div class="patientList" v-if="isPreviewed" >
                <div class="previewVideo" v-show="previewstate">
                    ...
                    <div class="closePhoneBtn" @mouseover="mouseoverPre" v-show="closePhoneBtnPre"  @click="closeVoiceBtnHandle">
                        <i class="el-icon-phone" ></i>
                    </div>
                </div>
                <div class="remoteVideo" v-show="!previewstate">
                    <video ref="remoteVideo"  autoplay muted playsinline controls @mouseover="mouseoverRem" @mouseleave="mouseleaveRem"></video>
        ...
        <div class="chatRight">
            <div class="chatHeaderName" v-if="isHistory">
                患者：{{name +'  '+ sexStr + '  '+ age }}
            ...
            <div class="panel" v-if="isHistory">
                <el-form :model="formData" :rules="rules" ref="chatForm">
                    <el-form-item >
                        <!-- prop="textMessage" -->
                        <!-- @keyup.native="handleWordMessageEvnt()" -->
                        <el-input type="textarea" class="messageInput"  :rows="4" resize="none"  v-model="formData.textMessage" @keyup.native="enterSendText()" @focus="changeValue(0)" placeholder="回复"></el-input>
    ...
    <div class="dialog" v-if="dialogVisible" @click.capture="handleMask">
        ...
        mounted() { 
            ...
            this.customerInfo = {
                data: {
                    customerId: 1,
                    customerName: "档案管理员",
                    headImg: require('../../../../assets/images/chat/patient.png'),
                    customerType: 0,
                    customerPhone: "",
                    createTime: "2019-12-06 10:00:40",
                    updateTime: "2019-12-25 11:42:24",
                    enabled: 1,
                    autoMessage: ""
                }
            }
        ...
        methods: {
            ...
            async handleParentEvent(e) {
                ...
                this.socket = new Socket({
                   ...
                   doctorId:this.dataSoket.doctorId,// this.customerInfo.customerId,
            ...
            monitorZeGo(){
                ...
                this.zg.on('roomUserUpdate', (roomID, updateType, userList) => {
                    ...
                    console.log('有人进出房间：',roomID);
                    // _that_.closeVoiceBtnHandle();
            ...
            async checkHttp(){
                var xmlhttp;
                let that = this;
                console.log('初始化zego必填数据：',this.rtc_config.appid)
                //初始化音频
                ...
                console.log('请求token前测试：',this.socketType+this.dataSoket.doctorId)
               let videoToken = await this.getToken({audioVideoType:1,idName:this.socketType+this.dataSoket.doctorId});
               if(videoToken.success){
                    that.zg.loginRoom(that.rtc_config.roomID, videoToken.data, { userID:'D'+this.dataSoket.doctorId, userName:that.rtc_config.nickName, }, { userUpdate: true });
                    this.localStream = await that.zg.createStream();
                    this.$refs.previewVideo.srcObject = that.localStream;
                    const result = that.zg.startPublishingStream(that.publishStreamId,that.localStream);
                    console.log('流ID：',that.localStream);
                    //录制接口
            ...
            agoraInit(){
                ...
                this.rtc.client.on("stream-removed", function (evt) {
                    var remoteStream = evt.stream;
                    var id = remoteStream.getId();
                    console.log('测试监听删除流：',remoteStream)
                    console.log('流删除了远程uid: ', id);
                    // 停止录制 --- 后台直接调第三方前端不处理
                })
            },
            //获取房间/频道ID
            getRoomId(data) {
                ...
                return this.$axios.get(this.showImageUrl+ajaxUrl,data);
            },
            //获取agora动态token
            getToken(data){
                ...
                if (ajaxData.success) {
                    if (ajaxData.data.length > 0) {
                        ...
                        ajaxData.data.map( v => {
                            ...
                            if (v.msgType == 'image') v.content = JSON.parse(v.content);
                        ...
                        console.log('首次加载滚动!!!!')
                            ...
                            // console.log('当前滚动位置：',top.target.scrollTop);
                            // console.log('测试滚动信息：',document.querySelector('.scroll-view'));
                ...
                if(type === '2'){
                    ...
                    if(this.previewstate){
                        ...
                        this.socket.onSend({
                            ...
                            // this.setTime     
                        console.log('记录时长：',this.duration)
            ...
            msgCallback(data) {
                ...
                console.log('接收服务器消息推送来的：',msgItem)
                if(msgItem.msgType != 'into_room'){
                    ...
                    // console.log('接收消息的高度：：：：',document.querySelector('.scroll-view').scrollHeight)
...
<style lang="scss" src="./chat1.scss"></style>
