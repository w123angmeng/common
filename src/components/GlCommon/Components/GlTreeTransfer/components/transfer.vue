  ...
  <div class="component-transfer">
      ...
      <div class="transfer-left">
          <h3 class="transfer-title">
              ...
              ></el-checkbox>
              <span>{{ fromTitle }}</span>
              <span v-if="countStatistics" style="float: right" class="title-left">{{from_check_keys_count.length}}/{{self_from_data_flat_count.length}}</span>
          ...
          <div class="transfer-main">
              ...
              <el-tree
                  ...
                  :default-expand-all="leftOpenAll"
    ...
    <div class="transfer-center">
      ...
      <template v-else>
        <p class="transfer-center-item">
          <el-button
            type="primary"
            @click="removeToSource"
            :disabled="to_disabled || cus_to_disabled"
            icon="el-icon-arrow-right"
            circle
          ></el-button>
        </p>
        <p class="transfer-center-item">
          <el-button
              ...
              circle
              :disabled="from_disabled || cus_from_disabled"
          ></el-button>
        </p>
      </template>
    </div>
      ...
      <div class="transfer-right">
          <h3 class="transfer-title">
              ...
              ></el-checkbox>
              <span>{{ toTitle }}</span>
              <span v-if="countStatistics" style="float: right" class="title-left">{{to_check_keys_count.length}}/{{self_to_data_flat_count.length}}</span>
          ...
          <div class="transfer-main">
              ...
              <el-tree
                  ...
                  :load="rightloadNode"
                  :props="selfDefaultProps"
                  :default-expand-all="rightOpenAll"
                  :highlight-current="highLight"
                  :check-strictly="checkStrictly"
...
export default {
  ...
  data() {
    return {
      from_check_keys_count: [], // 源数据选中数量
      to_check_keys_count: [], // 目标数据选中数量
  ...
  computed: {
    // 左侧拉平总数
    self_from_data_flat_count() {
        return flattenDeep(this.self_from_data, this.selfDefaultProps.children) || []
    },
    // 右侧拉平总数
    self_to_data_flat_count() {
      return flattenDeep(this.self_to_data, this.selfDefaultProps.children) || []
    },
    // 左侧数据
    self_from_data() {
  ...
  watch: {
    ...
    from_check_keys(val) {
      ...
      this.from_check_keys_count = this.$refs["from-tree"] && this.$refs["from-tree"].getCheckedKeys() || []
    },
    // 右侧 状态监测
    to_check_keys(val) {
      if (val.length > 0) {
        ...
        this.to_check_keys_count = this.$refs["to-tree"] && this.$refs["to-tree"].getCheckedKeys() || []
    },
    // 左侧 数据筛选
    filterFrom(val) {
  ...
  methods: {
    ...
    fromAllBoxChange(val) {
      ...
      if (val) {
        ...
        // this.from_check_keys = this.$refs["from-tree"].getCheckedKeys();
      } else {
    ...
    toAllBoxChange(val) {
      ...
      if (val) {
        ...
        this.$refs["to-tree"].setCheckedNodes(this.to_check_keys);
          // this.to_check_keys = this.$refs["to-tree"].getCheckedKeys();
      } else {
